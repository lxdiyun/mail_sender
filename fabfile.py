"""
Starter fabfile for deploying the mail_sender project.
...
# CHANGEME
env.hosts = ['user@mail_sender.example.com']
env.code_dir = '/srv/www/mail_sender'
env.project_dir = '/srv/www/mail_sender/mail_sender'
env.static_root = '/srv/www/mail_sender/static/'
env.virtualenv = '/srv/www/mail_sender/.virtualenv'
env.code_repo = 'git@github.com:user/mail_sender.git'
env.django_settings_module = 'mail_sender.settings'
...
def restart():
    ...
    with cd(env.code_dir):
        run("touch %s/mail_sender/wsgi.py" % env.code_dir)
